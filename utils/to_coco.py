...
def convert_bbox_format(bboxes, image_size):
    """
...
def get_df2(label_dir, image_dir, dir_of='train'):
    df = []
    for filename in os.listdir(label_dir):
        if filename.endswith('.txt'):
            filepath = os.path.join(label_dir, filename)
    ...
    classes = sorted(df['class'].unique())
    category_mapping = {cls: idx + 1 for idx, cls in enumerate(classes)}
    categories = [{"id": idx, "name": cls} for cls, idx in category_mapping.items()]
    coco["categories"] = categories
    ...
    for _, row in df.iterrows():
        ...
        annotation = {
            ...
            "image_id": images_dict[img_name],
            "category_id": category_mapping[row['class']],
            "bbox": [row['xmin'], row['ymin'], bbox_width, bbox_height],  # Correct format
            "area": area,
            "iscrowd": 0
        }
        coco["annotations"].append(annotation)
